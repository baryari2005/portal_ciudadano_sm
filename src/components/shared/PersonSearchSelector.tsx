...
/* eslint-disable @next/next/no-img-element -- Las fotos de identidad y avatares provienen de URLs firmadas temporales. */
...
export function PersonSearchSelector<T extends PersonSearchOption>({ value, onChange, search, identifyQr, title = "Buscar persona", searchPlaceholder = "Nombre, apellido o DNI", compactMobileSelected = false }: Props<T>) {
  ...
  const identify = useCallback(async (token: string) => {
    ...
    try {
      ...
      setQrFeedback("idle");
      toast.error(getAxiosMessage(error, "No pudimos identificar el QR."));
    } finally {
