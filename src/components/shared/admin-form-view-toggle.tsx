...
export function AdminFormViewToggle({ value, onChange, compactOnMobile = false }: { value: "workflow" | "full"; onChange: (value: "workflow" | "full") => void; compactOnMobile?: boolean }) {
  return (
    <section className={`${compactOnMobile ? "p-1 sm:p-4" : "p-4"} mb-5 rounded-2xl border border-[var(--brand-border)] bg-white/75 shadow-sm sm:flex sm:items-center sm:justify-between sm:gap-6`}>
      ...
      <div className={`${compactOnMobile ? "mt-0" : "mt-4"} grid shrink-0 grid-cols-2 gap-2 sm:mt-0`} aria-label="Modo de visualización">
