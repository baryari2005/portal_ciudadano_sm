...
export function AdminWorkflowLayout({
  ...
  return (
    <div className={fullWidth ? "grid items-start gap-5" : "grid items-start gap-5 lg:grid-cols-[190px_minmax(0,1fr)] xl:grid-cols-[220px_minmax(0,1fr)] 2xl:grid-cols-[260px_minmax(0,1fr)] 2xl:gap-6"}>
      {!fullWidth ? <aside className="h-fit self-start rounded-3xl bg-[var(--brand-primary)] p-4 text-white shadow-sm lg:sticky lg:top-0">
          ...
          {sections.map(({ id, label, icon: Icon, status }) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-12 items-center gap-3 rounded-xl px-3 py-2 text-left text-sm font-bold transition-colors ${active ? "bg-[#DDF28A] text-[var(--brand-ink)]" : status === "unsaved" ? "bg-amber-300/20 text-amber-100" : "hover:bg-white/10"}`}
              >
