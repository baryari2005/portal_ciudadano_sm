...
export function GoogleAddressInput({id,value,placeId,onChange,className,disabled,placeholder}:Props){
  ...
  const manual=<div className="relative"><MapPin className="pointer-events-none absolute left-3 top-1/2 z-10 size-4 -translate-y-1/2 text-[#1D4F36]"/><Input id={id} value={value} disabled={disabled} onChange={event=>onChange({address:event.target.value,placeId:null,lat:null,lng:null})} className={className} placeholder={placeholder}/></div>;
  return <div>{mode==="manual"?manual:<div className="relative"><MapPin className="pointer-events-none absolute left-3 top-1/2 z-10 size-4 -translate-y-1/2 text-[#1D4F36]"/><div ref={hostRef} className={mode==="loading"?"min-h-11 animate-pulse rounded-xl bg-[#EEF6E9]":""}/></div>}{placeId?<p className="mt-1 flex items-center gap-1 text-xs font-bold text-[#1D4F36]"><CheckCircle2 className="size-3.5"/>Dirección validada por Google</p>:mode==="manual"?<p className="mt-1 text-xs font-medium text-[#5F6F68]">Ingreso manual disponible.</p>:<p className="mt-1 text-xs font-medium text-[#5F6F68]">Seleccioná una sugerencia para validar la dirección.</p>}</div>
}
