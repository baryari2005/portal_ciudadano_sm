...
import { OpenStreetMapAddressPicker } from "./OpenStreetMapAddressPicker";
import type { AddressLocation } from "@/features/geocoding/types/location.types";

export type GoogleAddressValue=AddressLocation;
type Props={id:string;value:string;placeId?:string|null;lat?:number|null;lng?:number|null;locality?:string|null;province?:string|null;postalCode?:string|null;display?:"full"|"input"|"map";onChange:(value:GoogleAddressValue)=>void;className?:string;disabled?:boolean;placeholder?:string};
type GooglePlace = {
  id?: string;
...
type GooglePlacesLibrary = {
  ...
  }) => GoogleAutocompleteElement;
};
type GoogleMapsApi = {
  maps: { importLibrary: (library: "places") => Promise<GooglePlacesLibrary> };
};
declare global{interface Window{google?:GoogleMapsApi;__googleMapsPromise?:Promise<void>}}

function loadGoogleMaps(key:string){if(window.google?.maps?.importLibrary)return Promise.resolve();if(window.__googleMapsPromise)return window.__googleMapsPromise;window.__googleMapsPromise=new Promise<void>((resolve,reject)=>{const callback=`__googleMapsReady_${Date.now()}`;Object.defineProperty(window,callback,{configurable:true,value:()=>{Reflect.deleteProperty(window,callback);resolve()}});const script=document.createElement("script");script.src=`https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(key)}&loading=async&v=weekly&language=es&region=AR&auth_referrer_policy=origin&callback=${callback}`;script.async=true;script.onerror=()=>reject(new Error("No se pudo cargar Google Maps"));document.head.appendChild(script)});return window.__googleMapsPromise}

function LegacyGoogleAddressInput({id,value,placeId,onChange,className,disabled,placeholder}:Props){
  const hostRef=useRef<HTMLDivElement|null>(null),widgetRef=useRef<GoogleAutocompleteElement|null>(null),callbackRef=useRef(onChange),valueRef=useRef(value),disabledRef=useRef(disabled);callbackRef.current=onChange;valueRef.current=value;disabledRef.current=disabled;
  const apiKey=process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
  const[mode,setMode]=useState<"loading"|"google"|"manual">(apiKey?"loading":"manual");

  useEffect(()=>{if(!apiKey||!hostRef.current)return;let active=true;let widget:GoogleAutocompleteElement|undefined;void loadGoogleMaps(apiKey).then(async()=>{const google=window.google;if(!google)throw new Error("Google Maps no está disponible");const places=await google.maps.importLibrary("places");if(!active||!hostRef.current)return;widget=new places.PlaceAutocompleteElement({includedRegionCodes:["ar"],requestedLanguage:"es",requestedRegion:"ar"});widget.id=id;widget.placeholder=placeholder??"Ingresá una dirección";widget.value=valueRef.current;widget.disabled=Boolean(disabledRef.current);widget.noInputIcon=true;widget.className="google-address-autocomplete";widget.addEventListener("input",()=>callbackRef.current({address:String(widget?.value??""),placeId:null,lat:null,lng:null}));widget.addEventListener("gmp-select",async(event:Event)=>{const place=(event as GooglePlaceSelectEvent).placePrediction.toPlace();await place.fetchFields({fields:["id","formattedAddress","location"]});const location=place.location;callbackRef.current({address:place.formattedAddress||String(widget?.value??""),placeId:place.id||null,lat:location?.lat()??null,lng:location?.lng()??null})});widget.addEventListener("gmp-error",()=>setMode("manual"));hostRef.current.replaceChildren(widget);widgetRef.current=widget;setMode("google")}).catch(()=>setMode("manual"));return()=>{active=false;widget?.remove();widgetRef.current=null}},[apiKey,id,placeholder]);
...
export function GoogleAddressInput(props:Props){
  if((process.env.NEXT_PUBLIC_MAP_PROVIDER??"openstreetmap").toLowerCase()==="google")return <LegacyGoogleAddressInput {...props}/>;
  return <OpenStreetMapAddressPicker {...props}/>;
}
