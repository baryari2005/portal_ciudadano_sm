...
export default function RequestAccessStatusPage() {
  ...
  return <main className="min-h-dvh bg-[var(--brand-page)]">
    <header className="border-b border-white/15 bg-primary"><div className="mx-auto flex h-[88px] items-center justify-between gap-6 px-5 sm:px-8 lg:px-10"><div className="flex min-w-0 items-center gap-5 text-white"><Image src="/logoentero.png" alt="MAS San Miguel" width={112} height={52} priority className="h-auto w-24 object-contain brightness-0 invert sm:w-28"/><div className="hidden h-14 w-px bg-white/45 sm:block"/><div><p className="text-base font-bold sm:text-lg">Portal ciudadano</p><p className="mt-1 text-sm font-medium text-[#D8E178] sm:text-base">Sistema de Ayuda<span className="block">y Actividades</span></p></div></div><Link href="/login" className="hidden items-center gap-3 font-bold text-white sm:flex"><span className="grid size-11 place-items-center rounded-lg bg-[#e9f3d8] text-[var(--brand-heading)]"><User className="size-7 fill-current"/></span>Iniciar sesión</Link></div></header>

    <section className="mx-auto w-full max-w-5xl px-4 py-7 sm:px-6 sm:py-9">
      <header className="mb-6 border-b border-[var(--brand-border)] pb-5"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-[var(--brand-border-soft)] text-[var(--brand-primary)]"><ClipboardCheck className="size-6"/></span><h1 className="text-3xl font-bold tracking-tight text-[var(--brand-primary)] sm:text-4xl">Estado de la solicitud</h1></div><p className="mt-3 flex items-start gap-2 text-sm text-[var(--brand-muted)] sm:text-base"><Info className="mt-0.5 size-5 shrink-0 text-[var(--brand-secondary)]"/>Consultá el resultado y los próximos pasos de tu solicitud de acceso.</p></header>

      <section className="rounded-3xl border border-[var(--brand-secondary)]/20 bg-white/80 p-5 text-[var(--brand-ink)] shadow-sm sm:p-6 lg:p-8">
