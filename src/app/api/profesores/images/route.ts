...
export async function POST(req: NextRequest) {
  try {
    ...
    const bucket = process.env.SUPABASE_BUCKET_TEACHER_IMAGES || process.env.SUPABASE_BUCKET_AVATARS || "avatars";
    const path = `professors/${randomUUID()}.${extension}`;
    const { error } = await supabaseAdmin.storage.from(bucket).upload(path, content, {
      contentType: file.type,
      upsert: false,
    });
    if (error) throw new Error("PROFESSOR_IMAGE_UPLOAD_FAILED");
    const { data } = supabaseAdmin.storage.from(bucket).getPublicUrl(path);
    return NextResponse.json({ data: { path, publicUrl: data.publicUrl } }, { status: 201 });
  } catch (error) {
    return mapApiRouteError(error, "No pudimos subir la foto del profesor.");
  }
}
