...
export async function POST(req: NextRequest) {
  try {
    ...
    const canUpload = user.permisos.some((permission) => permission.modulo === "actividades" && ["crear", "editar"].includes(permission.accion));
    ...
    const path = `requirements/${randomUUID()}.${extension}`;
    ...
    if (error) throw new Error("REQUIREMENT_IMAGE_UPLOAD_FAILED");
    const { data } = supabaseAdmin.storage.from(bucket).getPublicUrl(path);
    return NextResponse.json({ data: { path, publicUrl: data.publicUrl } }, { status: 201 });
  } catch (error) {
    return mapApiRouteError(error, "No pudimos subir la imagen del requisito.");
  }
}
