...
export async function GET(request: NextRequest) {
  try {
    const user = await requireAuth(request), enrollmentId = request.nextUrl.searchParams.get("enrollmentId") ?? "", establishmentId = request.nextUrl.searchParams.get("establishmentId") ?? undefined;
