...
export async function GET(req:NextRequest,{params}:P){try{const user=await requireAuth(req);requirePermission(user,"activity_sessions","ver");return NextResponse.json({data:await getTeacherClass(user.id,(await params).sessionId,req.nextUrl.searchParams.get("establishmentId")??undefined)});}catch(error){return mapApiRouteError(error,"No pudimos cargar la clase.")}}
