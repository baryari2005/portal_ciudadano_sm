...
export default function EditResourcePage() {
  ...
  return <>{loading ? <CatalogLoadingState label="formulario de recurso" fullPage /> : null}{initial ? <div className={loading ? "hidden" : undefined}><div className="grid min-h-[calc(100dvh-var(--topbar-h)-48px)] w-full grid-rows-[auto_minmax(0,1fr)] gap-5 bg-[var(--brand-page)] p-4 sm:p-6 lg:h-[calc(100dvh-var(--topbar-h)-48px)] lg:overflow-hidden lg:p-8"><AdminFormHeader icon={Boxes} title="Editar recurso físico" description="Actualizá la identificación, capacidad o disponibilidad del recurso." className="mb-0" /><div className="min-h-0 overflow-y-auto pr-2"><ResourceForm mode="edit" defaultValues={initial} onLoadingChange={setFormLoading} /></div></div></div> : null}</>;
}
