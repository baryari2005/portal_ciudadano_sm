...
export default function NewResourcePage() {
  ...
  return <>{loading ? <CatalogLoadingState label="formulario de recurso" fullPage /> : null}<div className={loading ? "hidden" : undefined}><div className="grid min-h-[calc(100dvh-var(--topbar-h)-48px)] w-full grid-rows-[auto_minmax(0,1fr)] gap-5 bg-[#F7FBF5] p-4 sm:p-6 lg:h-[calc(100dvh-var(--topbar-h)-48px)] lg:overflow-hidden lg:p-8"><AdminFormHeader icon={Boxes} title="Alta de recurso físico" description="Cargá el recurso, su establecimiento y la forma en que participa de las reservas." className="mb-0" /><div className="min-h-0 overflow-y-auto pr-2"><ResourceForm mode="create" onLoadingChange={setLoading} /></div></div></div></>;
}
