...
export function ResourcesPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const data = await listResourcesClient(); setItems(data); setSelectedId((current) => current || data[0]?.id || ""); } catch (error) { toast.error(error instanceof Error ? error.message : "No pudimos cargar los recursos."); } finally { setLoading(false); } }, []);
