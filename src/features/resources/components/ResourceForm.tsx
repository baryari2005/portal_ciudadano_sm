...
const inputClass = "h-11 rounded-xl border-[#C9D9C3] bg-[#F7FBF5] font-medium text-[#173C2A] placeholder:text-[#6D8D75]";
const selectClass = "h-11 rounded-xl border border-[#C9D9C3] bg-[#F7FBF5] pl-9 text-sm font-medium text-[#173C2A] shadow-sm";
...
export function ResourceForm({ mode, defaultValues, onLoadingChange }: { mode: "create" | "edit"; defaultValues?: Resource | null; onLoadingChange?: (loading: boolean) => void }) {
  ...
  if (loading) return onLoadingChange ? null : <div className="flex min-h-72 items-center justify-center rounded-3xl border border-[#819B56]/20 bg-white/80"><Loader2 className="mr-3 animate-spin text-[#1D4F36]" /><span className="font-semibold text-[#1D4F36]">Cargando formulario...</span></div>;

  return <form id="resource-form" className="w-full" onSubmit={submit}>
    <div className="rounded-3xl border border-[#819B56]/20 bg-white/80 p-5 text-[#173C2A] shadow-sm sm:p-6 lg:p-8">
      <div className="mb-6 flex items-center justify-between border-b border-[#C9D9C3] pb-5"><div><h2 className="text-lg font-extrabold text-[#003A22]">Datos del recurso físico</h2><p className="mt-1 text-sm font-medium text-[#5F6F68]">Información requerida para identificar el recurso y controlar su disponibilidad.</p></div></div>
      <div className="space-y-6"><div className="grid gap-x-8 gap-y-4 sm:grid-cols-2">
        ...
        <Field label="Establecimiento *" icon={Building2} error={errors.establecimientoId}><Select value={form.establecimientoId} onValueChange={(value) => set("establecimientoId", value)}><SelectTrigger className={`${selectClass} w-full`}><SelectValue placeholder="Seleccionar establecimiento" /></SelectTrigger><SelectContent className="border-[#C9D9C3] bg-[#F7FBF5]">{establishments.map((item) => <SelectItem key={item.id} value={item.id}>{item.nombre} · {item.direccion}</SelectItem>)}</SelectContent></Select></Field>
        <Field label="Tipo de recurso *" icon={Boxes}><Select value={form.tipo} onValueChange={(value) => set("tipo", value as ResourceInput["tipo"])}><SelectTrigger className={`${selectClass} w-full`}><SelectValue /></SelectTrigger><SelectContent className="border-[#C9D9C3] bg-[#F7FBF5]"><SelectItem value="ESPACIO">Espacio</SelectItem><SelectItem value="CANCHA">Cancha</SelectItem><SelectItem value="EQUIPAMIENTO">Equipamiento</SelectItem><SelectItem value="COMPUTADORA">Computadora</SelectItem><SelectItem value="ANDARIVEL">Andarivel</SelectItem><SelectItem value="OTRO">Otro</SelectItem></SelectContent></Select></Field>
        <Field label="Forma de reserva *" icon={Layers3}><Select value={form.modoReserva} onValueChange={(value) => set("modoReserva", value as ResourceInput["modoReserva"])}><SelectTrigger className={`${selectClass} w-full`}><SelectValue /></SelectTrigger><SelectContent className="border-[#C9D9C3] bg-[#F7FBF5]"><SelectItem value="CAPACIDAD">Capacidad compartida</SelectItem><SelectItem value="ESPECIFICO">Recurso específico</SelectItem><SelectItem value="EXCLUSIVO">Uso exclusivo</SelectItem></SelectContent></Select></Field>
        <Field label="Unidades disponibles *" icon={Layers3} error={errors.capacidadUnidades}><Input className={inputClass} type="number" min={1} value={form.capacidadUnidades} onChange={(e) => set("capacidadUnidades", Number(e.target.value))} /><p className="mt-1 text-xs text-[#5F6F68]">En recursos exclusivos o individuales normalmente es 1.</p></Field>
        <Field label="Descripción" icon={NotebookText}><Textarea className="min-h-11 resize-y rounded-xl border-[#C9D9C3] bg-[#F7FBF5] pl-9 font-medium text-[#173C2A]" value={form.descripcion ?? ""} onChange={(e) => set("descripcion", e.target.value)} placeholder="Ubicación, características o indicaciones de uso." /></Field>
      </div>
      {mode === "create" ? <section className="space-y-4 border-t border-[#D7E0D8] pt-6"><div><p className="text-sm font-bold uppercase tracking-normal text-[#1D4F36]">Cómo cargar el recurso</p><p className="mt-1 text-sm text-[#5F6F68]">Elegí el criterio que represente cómo se utiliza físicamente.</p></div><div className="grid gap-3 md:grid-cols-3"><Guide title="Capacidad compartida">Un conjunto como “Computadoras del ciber”, indicando 12 unidades.</Guide><Guide title="Recurso específico">Cada elemento identificado, como “Computadora 01”.</Guide><Guide title="Uso exclusivo">Una cancha o salón que queda bloqueado durante la reserva.</Guide></div></section> : null}
      </div>
    </div>
    <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button asChild type="button" variant="outline" className="h-12 w-full justify-center gap-3 rounded-xl border-[#C9D9C3] bg-[#F7FBF5] px-8 text-base font-bold text-[#173C2A] shadow-sm hover:bg-[#EEF6E9] sm:w-auto"><Link href="/resources"><ArrowLeft className="size-5" />Volver</Link></Button><Button type="submit" size="lg" disabled={saving} className="h-12 w-full justify-center gap-3 rounded-xl bg-[#014D31] px-8 text-base font-bold text-white shadow-sm hover:bg-[#003A22] sm:w-auto">{saving ? <span className="inline-flex items-center gap-2"><Loader2 className="size-4 animate-spin" />Guardando...</span> : <span className="inline-flex items-center gap-2"><Save className="size-4" />{isEdit ? "Guardar cambios" : "Crear recurso"}</span>}</Button></div>
  </form>;
}
...
function Field({ label, icon: Icon, error, children }: { label: string; icon: typeof Boxes; error?: string; children: React.ReactNode }) { return <div className="space-y-1"><Label className="font-extrabold text-[#173C2A]">{label}</Label><div className="relative"><Icon className="pointer-events-none absolute left-3 top-[14px] z-10 size-4 text-[#1D4F36]" /><div className="[&_input]:pl-9">{children}</div></div>{error ? <p className="text-xs text-red-700">{error}</p> : null}</div>; }
function Guide({ title, children }: { title: string; children: React.ReactNode }) { return <div className="rounded-[18px] border border-[#D7E0D8] bg-[#F7FBF5] p-4"><p className="font-extrabold text-[#1D4F36]">{title}</p><p className="mt-1 text-sm text-[#5F6F68]">{children}</p></div>; }
