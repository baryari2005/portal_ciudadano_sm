...
export function EnrollmentsPage({ userId: userIdProp, embedded = false, onLoadingChange }: { userId?: string; embedded?: boolean; onLoadingChange?: (loading: boolean) => void } = {}) {
  ...
  return <AdminPageShell>
    <CatalogPageHeader icon={ClipboardCheck} title="Inscripciones" description="Administrá cupos, confirmaciones y listas de espera de las actividades." total={meta.total} createLabel="Nueva inscripción" canCreate={canCreate} onCreate={() => router.push("/enrollments/new")} />
    ...
    </AdminListPane>} detail={<div className={!selectedId ? "hidden lg:block" : undefined}><Detail item={selected} canEdit={canEdit} canViewDocuments={canViewDocuments} canState={canDelete || canAssign} onBack={() => setSelectedId(null)} onEdit={() => selected && router.push(`/enrollments/${selected.id}/edit`)} onChange={change} /></div>} />
  </AdminPageShell>;
}
