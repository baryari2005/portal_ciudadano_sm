...
type EnrollmentConflictScope = { scheduleId:string; day:string; startTime:string; endTime:string };
...
async function assertNoEnrollmentConflicts(tx:Prisma.TransactionClient,userId:string,requestedScopes:EnrollmentConflictScope[],excludeEnrollmentId?:string){
  ...
  const existing=await tx.inscripcion.findMany({where:{id:excludeEnrollmentId?{not:excludeEnrollmentId}:undefined,usuarioId:userId,estado:{in:["PENDIENTE","CONFIRMADA","LISTA_ESPERA"]},modalidad:{not:"POR_CLASE"}},select:{horarioActividad:{select:{id:true,diaSemana:true,horaInicio:true,horaFin:true,actividad:{select:{nombre:true}}}},horarios:{select:{horarioActividadId:true,horaInicio:true,horaFin:true,horarioActividad:{select:{diaSemana:true,horaInicio:true,horaFin:true,actividad:{select:{nombre:true}}}}}}}});
...
export async function createEnrollment(input: CreateEnrollmentInput, options: { notifyAdmin?: boolean } = {}) {
  try{return await serializable(async (tx) => {
    ...
    if (classId) {
      ...
      const session = await tx.claseActividad.findUnique({ where: { id: classId }, select: { fecha:true,horaFin:true,estado:true,horarioActividadId: true, horarioActividad: { select: { actividadId: true,estado:true,actividad:{select:{modalidadOperacion:true}} } } } });
