...
async function assertNoEnrollmentConflicts(tx:Prisma.TransactionClient,userId:string,requestedScopes:EnrollmentConflictScope[],excludeEnrollmentId?:string){
  for(let index=0;index<requestedScopes.length;index++)for(let compared=index+1;compared<requestedScopes.length;compared++){const current=requestedScopes[index],next=requestedScopes[compared];if(current.scheduleId===next.scheduleId)throw new CatalogConflictError("La persona ya posee una inscripción activa en este horario.");if(scopesOverlap(current,next))throw new CatalogConflictError("Los horarios seleccionados se superponen entre sí.");}
...
export async function getEnrollmentSlotAvailability(selections:Array<{horarioActividadId:string;horaInicio:string;horaFin:string}>,userId?:string,activityId?:string){return prisma.$transaction(async tx=>{const ids=[...new Set(selections.map(item=>item.horarioActividadId))],schedules=await tx.horarioActividad.findMany({where:{id:{in:ids}},include:{actividad:true}});if(schedules.length!==ids.length)throw new CatalogNotFoundError("Uno de los horarios no existe.");if(activityId&&schedules.some(schedule=>schedule.actividadId!==activityId))throw new CatalogValidationError("Uno de los horarios no pertenece a la actividad.");if(userId){const scopes=selections.map(selection=>{const schedule=schedules.find(item=>item.id===selection.horarioActividadId)!;return{scheduleId:schedule.id,day:schedule.diaSemana,startTime:selection.horaInicio,endTime:selection.horaFin};});await assertNoEnrollmentConflicts(tx,userId,scopes);}return Promise.all(selections.map(async selection=>{const schedule=schedules.find(item=>item.id===selection.horarioActividadId)!;const result=await capacity(tx,schedule,{horaInicio:selection.horaInicio});return{...selection,available:result.availableCount,status:result.availableCount>0?"DISPONIBLE":schedule.permiteListaEspera?"LISTA_ESPERA":"SIN_CUPO"};}));});}
