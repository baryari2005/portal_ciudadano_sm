...
export async function getEnrollmentSlotAvailability(selections:Array<{horarioActividadId:string;horaInicio:string;horaFin:string}>,userId?:string,activityId?:string){return prisma.$transaction(async tx=>{const ids=[...new Set(selections.map(item=>item.horarioActividadId))],schedules=await tx.horarioActividad.findMany({where:{id:{in:ids}},include:{actividad:true}});if(schedules.length!==ids.length)throw new CatalogNotFoundError("Uno de los horarios no existe.");if(activityId&&schedules.some(schedule=>schedule.actividadId!==activityId))throw new CatalogValidationError("Uno de los horarios no pertenece a la actividad.");if(userId&&activityId){const existingActivity=await tx.inscripcion.findFirst({where:{usuarioId:userId,horarioActividad:{actividadId:activityId}},orderBy:{createdAt:"desc"},select:{estado:true,modalidad:true,fechaFin:true,horarioActividadId:true,horarios:{select:{horarioActividadId:true,horaInicio:true,horaFin:true,horarioActividad:{select:{diaSemana:true,horaInicio:true,horaFin:true}}}},horarioActividad:{select:{diaSemana:true,horaInicio:true,horaFin:true}}}}),expiredPeriod=existingActivity?.modalidad==="POR_PERIODO"&&existingActivity.fechaFin&&existingActivity.fechaFin<new Date();const scopes=selections.map(selection=>{const schedule=schedules.find(item=>item.id===selection.horarioActividadId)!;return{day:schedule.diaSemana,startTime:selection.horaInicio,endTime:selection.horaFin};});if(existingActivity&&!expiredPeriod&&["PENDIENTE","CONFIRMADA","LISTA_ESPERA"].includes(existingActivity.estado)){const existingScopes=existingActivity.horarios.length?existingActivity.horarios.map(item=>({day:item.horarioActividad.diaSemana,startTime:item.horaInicio??item.horarioActividad.horaInicio,endTime:item.horaFin??item.horarioActividad.horaFin})):[{day:existingActivity.horarioActividad.diaSemana,startTime:existingActivity.horarioActividad.horaInicio,endTime:existingActivity.horarioActividad.horaFin}];const duplicate=scopes.find(requested=>existingScopes.some(current=>current.day===requested.day&&minutes(requested.startTime)<minutes(current.endTime)&&minutes(current.startTime)<minutes(requested.endTime)));if(duplicate)throw new CatalogConflictError("La persona ya está inscripta en esta actividad el mismo día y horario.");}await assertNoRecurringOverlap(tx,userId,activityId,scopes);}return Promise.all(selections.map(async selection=>{const schedule=schedules.find(item=>item.id===selection.horarioActividadId)!;const result=await capacity(tx,schedule,{horaInicio:selection.horaInicio});return{...selection,available:result.availableCount,status:result.availableCount>0?"DISPONIBLE":schedule.permiteListaEspera?"LISTA_ESPERA":"SIN_CUPO"};}));});}
...
export async function changeCitizenEnrollmentSchedules(userId:string,enrollmentId:string,selections:Array<{horarioActividadId:string;horaInicio:string;horaFin:string}>,options:{notifyAdministrators?:boolean}={}){
  return serializable(async tx=>{
    ...
    await assertNoRecurringOverlap(tx,userId,current.horarioActividad.actividad.id,selections.map(selection=>{const source=schedules.find(item=>item.id===selection.horarioActividadId)!;return{day:source.diaSemana,startTime:selection.horaInicio,endTime:selection.horaFin};}));
