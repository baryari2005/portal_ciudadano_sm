...
const scheduleSummary={include:{actividad:{select:{id:true,nombre:true,imagenUrl:true}},establecimiento:{select:{id:true,nombre:true}}}} as const;
const include={usuario:{select:{id:true,userId:true,nombre:true,apellido:true,documento:true,email:true,avatarUrl:true,deletedAt:true}},horarioActividad:scheduleSummary,horarios:{include:{horarioActividad:scheduleSummary},orderBy:{horarioActividad:{diaSemana:"asc"}}}} satisfies Prisma.InscripcionInclude;
...
export type EnrollmentConflictScope = { scheduleId:string; day:string; startTime:string; endTime:string };
const scopesOverlap=(left:EnrollmentConflictScope,right:EnrollmentConflictScope)=>left.day===right.day&&minutes(left.startTime)<minutes(right.endTime)&&minutes(right.startTime)<minutes(left.endTime);
export async function assertNoEnrollmentConflicts(tx:Prisma.TransactionClient,userId:string,requestedScopes:EnrollmentConflictScope[],excludeEnrollmentId?:string){
  for(let index=0;index<requestedScopes.length;index++)for(let compared=index+1;compared<requestedScopes.length;compared++){const current=requestedScopes[index],next=requestedScopes[compared];if(current.scheduleId===next.scheduleId&&current.startTime===next.startTime&&current.endTime===next.endTime)throw new CatalogConflictError("El mismo horario fue seleccionado más de una vez.");if(scopesOverlap(current,next))throw new CatalogConflictError("Los horarios seleccionados se superponen entre sí.");}
  const existing=await tx.inscripcion.findMany({where:{id:excludeEnrollmentId?{not:excludeEnrollmentId}:undefined,usuarioId:userId,estado:{in:["PENDIENTE","CONFIRMADA","LISTA_ESPERA"]}},select:{horarioActividad:{select:{id:true,diaSemana:true,horaInicio:true,horaFin:true,actividad:{select:{nombre:true}}}},horarios:{select:{horarioActividadId:true,horaInicio:true,horaFin:true,horarioActividad:{select:{diaSemana:true,horaInicio:true,horaFin:true,actividad:{select:{nombre:true}}}}}}}});
...
export async function createEnrollment(input: CreateEnrollmentInput, options: { notifyAdmin?: boolean; notifyMissingDocuments?: boolean } = {}) {
  try{return await serializable(async (tx) => {
    ...
    if (classId) {
      ...
      const session = await tx.claseActividad.findUnique({ where: { id: classId }, select: { fecha:true,horaInicio:true,horaFin:true,estado:true,horarioActividadId: true, horarioActividad: { select: { actividadId: true,estado:true,actividad:{select:{modalidadOperacion:true}} } } } });
      if (!session) throw new CatalogValidationError("La clase seleccionada no existe.");
      const sessionEndsAt=new Date(`${session.fecha.toISOString().slice(0,10)}T${session.horaFin}:00-03:00`),invalidSession=!['PROGRAMADA','EN_CURSO'].includes(session.estado)||session.horarioActividad.estado!=="ACTIVO"||sessionEndsAt<=new Date();
      if(invalidSession)throw new CatalogValidationError(session.horarioActividad.actividad.modalidadOperacion==="EVENTO_UNICO"?"El evento no tiene una fecha u horario válido para inscripciones.":"La clase seleccionada no admite nuevas inscripciones.");
      ...
      requestedSlots.push({ horarioActividadId: session.horarioActividadId, horaInicio: session.horaInicio, horaFin: session.horaFin });
    }
    if (activityId) {
    ...
    {const scopes=requestedSlots.length?requestedSlots.map(slot=>{const source=schedules.find(item=>item.id===slot.horarioActividadId)!;return{scheduleId:source.id,day:source.diaSemana,startTime:slot.horaInicio,endTime:slot.horaFin};}):schedules.map(item=>({scheduleId:item.id,day:item.diaSemana,startTime:item.horaInicio,endTime:item.horaFin}));await assertNoEnrollmentConflicts(tx,input.usuarioId,scopes);}
    ...
    const now = new Date();
    const existing = await tx.inscripcion.findFirst({ where: { usuarioId: input.usuarioId, horarioActividadId: schedule.id }, orderBy: { createdAt: "desc" }, include });
    const expiredPeriod = existing?.modalidad === "POR_PERIODO" && existing.fechaFin && existing.fechaFin < now;
    if (existing && !expiredPeriod && !["CANCELADA", "RECHAZADA", "BAJA"].includes(existing.estado)) throw new CatalogConflictError("La persona ya posee una inscripción activa en este horario.");
...
export async function getEnrollmentSlotAvailability(selections:Array<{horarioActividadId:string;horaInicio:string;horaFin:string}>,userId?:string,activityId?:string,excludeEnrollmentId?:string){return prisma.$transaction(async tx=>{const ids=[...new Set(selections.map(item=>item.horarioActividadId))],schedules=await tx.horarioActividad.findMany({where:{id:{in:ids}},include:{actividad:true}});if(schedules.length!==ids.length)throw new CatalogNotFoundError("Uno de los horarios no existe.");if(activityId&&schedules.some(schedule=>schedule.actividadId!==activityId))throw new CatalogValidationError("Uno de los horarios no pertenece a la actividad.");if(userId){const scopes=selections.map(selection=>{const schedule=schedules.find(item=>item.id===selection.horarioActividadId)!;return{scheduleId:schedule.id,day:schedule.diaSemana,startTime:selection.horaInicio,endTime:selection.horaFin};});await assertNoEnrollmentConflicts(tx,userId,scopes,excludeEnrollmentId);}return Promise.all(selections.map(async selection=>{const schedule=schedules.find(item=>item.id===selection.horarioActividadId)!;const result=await capacity(tx,schedule,{horaInicio:selection.horaInicio});return{...selection,available:result.availableCount,status:result.availableCount>0?"DISPONIBLE":schedule.permiteListaEspera?"LISTA_ESPERA":"SIN_CUPO"};}));});}

export async function changeCitizenEnrollmentSchedules(userId:string,enrollmentId:string,selections:Array<{horarioActividadId:string;horaInicio:string;horaFin:string}>,options:{notifyAdministrators?:boolean}={}){
  return serializable(async tx=>{
    const current=await tx.inscripcion.findFirst({where:{id:enrollmentId,usuarioId:userId},include:{horarios:true,horarioActividad:{include:{actividad:{select:{id:true,nombre:true,modalidadOperacion:true}},establecimiento:{select:{id:true,nombre:true}}}},usuario:{select:{id:true,nombre:true,apellido:true,documento:true,email:true}}}});
    if(!current)throw new CatalogNotFoundError("Inscripción no encontrada.");
    if(!["PENDIENTE","CONFIRMADA","LISTA_ESPERA"].includes(current.estado))throw new CatalogValidationError("Esta inscripción no permite cambiar horarios.");
    if(current.horarioActividad.actividad.modalidadOperacion!=="TURNO_RECURRENTE")throw new CatalogValidationError("Solo las actividades con turnos recurrentes permiten cambiar horarios.");
    if(!selections.length)throw new CatalogValidationError("Seleccioná al menos un horario.");
    const ids=[...new Set(selections.map(item=>item.horarioActividadId))],schedules=await Promise.all(ids.map(id=>lockSchedule(tx,id)));
    if(schedules.some(schedule=>schedule.actividad.id!==current.horarioActividad.actividad.id))throw new CatalogValidationError("Uno de los horarios no pertenece a la actividad.");
    for(const selection of selections){const source=schedules.find(item=>item.id===selection.horarioActividadId)!;const start=minutes(selection.horaInicio),end=minutes(selection.horaFin),rangeStart=minutes(source.horaInicio),rangeEnd=minutes(source.horaFin);if(start<rangeStart||end>rangeEnd||start>=end||(source.duracionTurnoMinutos&&end-start!==source.duracionTurnoMinutos))throw new CatalogValidationError("Uno de los turnos no coincide con la franja configurada.");}
    await assertNoEnrollmentConflicts(tx,userId,selections.map(selection=>{const source=schedules.find(item=>item.id===selection.horarioActividadId)!;return{scheduleId:source.id,day:source.diaSemana,startTime:selection.horaInicio,endTime:selection.horaFin};}),current.id);
    if(current.estado==="CONFIRMADA"){const occupied=new Set((current.horarios.length?current.horarios:[{horarioActividadId:current.horarioActividadId,horaInicio:current.horarioActividad.horaInicio,horaFin:current.horarioActividad.horaFin}]).map(item=>`${item.horarioActividadId}-${item.horaInicio}-${item.horaFin}`));for(const selection of selections){const key=`${selection.horarioActividadId}-${selection.horaInicio}-${selection.horaFin}`;if(occupied.has(key))continue;const source=schedules.find(item=>item.id===selection.horarioActividadId)!;const cap=await capacity(tx,source,{horaInicio:selection.horaInicio});if(cap.availableCount<=0)throw new CatalogConflictError("Uno de los nuevos horarios ya no tiene cupo disponible.");}}
    const primary=schedules.find(item=>item.id===selections[0].horarioActividadId)!;
    const row=await tx.inscripcion.update({where:{id:current.id},data:{horarioActividadId:primary.id,horarios:{deleteMany:{},create:selections}},include});
    await createNotification({userId,type:"GENERAL",title:"Horarios actualizados",message:`Administración actualizó tus días u horarios de ${current.horarioActividad.actividad.nombre}.`,priority:"NORMAL",actionUrl:"/citizen/enrollments",actionLabel:"Ver inscripción",entityType:"enrollment",entityId:row.id,deduplicationKey:`enrollment-schedules-updated:${row.id}:${row.updatedAt.getTime()}`},tx);
    const citizenName=[current.usuario.nombre,current.usuario.apellido].filter(Boolean).join(" ")||current.usuario.documento||"Un ciudadano";
    if(options.notifyAdministrators!==false)await notifyAdministrators({senderId:userId,type:"GENERAL",title:"Horarios de inscripción modificados",message:`${citizenName} modificó sus días u horarios de ${current.horarioActividad.actividad.nombre}.`,priority:"NORMAL",actionUrl:`/enrollments?activityId=${current.horarioActividad.actividad.id}`,actionLabel:"Ver inscripción",entityType:"enrollment",entityId:row.id,deduplicationKey:`admin-enrollment-schedules-updated:${row.id}:${row.updatedAt.getTime()}`},tx);
    return map(tx,row);
  });
}
...
async function promote(tx:Prisma.TransactionClient,scheduleId:string){
  const schedule=await lockSchedule(tx,scheduleId),cap=await capacity(tx,schedule);
  if(cap.availableCount<=0)return;
  const waiting=await tx.inscripcion.findMany({where:{estado:"LISTA_ESPERA",OR:[{horarioActividadId:scheduleId},{horarios:{some:{horarioActividadId:scheduleId}}}]},include:{usuario:{select:{fechaNacimiento:true,genero:true}}},orderBy:[{fechaListaEspera:"asc"},{createdAt:"asc"}]});
  for(const candidate of waiting){
    const ageResult=evaluateEnrollmentAge({birthDate:candidate.usuario.fechaNacimiento,gender:candidate.usuario.genero,referenceDate:new Date(),audiences:schedule.actividad.publicosObjetivo.map((item)=>item.publicoObjetivo)}),documentsApproved=await hasApprovedRequiredDocuments(tx,candidate.usuarioId,schedule.actividad.requisitos.map((item)=>item.requisitoId));
    if(!ageResult.eligible||!documentsApproved)continue;
    const promotedAt=new Date(),promoted=await tx.inscripcion.update({where:{id:candidate.id},data:{estado:"CONFIRMADA",fechaConfirmacion:promotedAt},include}),remainingWaitlist=await tx.inscripcion.count({where:{estado:"LISTA_ESPERA",OR:[{horarioActividadId:scheduleId},{horarios:{some:{horarioActividadId:scheduleId}}}]}});
    ...
    await createAuditLogTx(tx,{action:"PROMOVER",entityType:"INSCRIPCION",entityId:promoted.id,entityName:`Inscripción en ${promoted.horarioActividad.actividad.nombre}`,changes:{estado:{before:"LISTA_ESPERA",after:"CONFIRMADA"}},metadata:{scheduleId,releasedSeat:true,promotedAt,remainingWaitlist},origin:"SISTEMA"});
    return;
  }
}
export async function updateEnrollment(id: string, input: UpdateEnrollmentInput) {
  return serializable(async (tx) => {
    ...
    if (["CONFIRMADA", "LISTA_ESPERA"].includes(target) && target !== current.estado) {
      ...
      {
        const scopes = current.horarios.length ? current.horarios.map((item) => ({ scheduleId:item.horarioActividadId,day: item.horarioActividad.diaSemana, startTime: item.horaInicio ?? item.horarioActividad.horaInicio, endTime: item.horaFin ?? item.horarioActividad.horaFin })) : [{ scheduleId:schedule.id,day: schedule.diaSemana, startTime: schedule.horaInicio, endTime: schedule.horaFin }];
        await assertNoEnrollmentConflicts(tx,current.usuarioId,scopes,current.id);
      }
    ...
    if (released) {
      const releasedScheduleIds=[...new Set(current.horarios.length?current.horarios.map((item)=>item.horarioActividadId):[current.horarioActividadId])];
      for(const releasedScheduleId of releasedScheduleIds)await promote(tx,releasedScheduleId);
    }
    ...
    if (input.estado && input.estado !== current.estado && ["CANCELADA", "RECHAZADA", "BAJA"].includes(target)) {
      ...
      const rejectionDetail = target === "RECHAZADA" && row.motivoRechazo ? ` Motivo: ${row.motivoRechazo}` : "";
      await createNotification({ userId: row.usuarioId, type: config.type, title: config.title, message: `Tu inscripción a ${row.horarioActividad.actividad.nombre} fue ${target === "BAJA" ? "dada de baja" : target.toLowerCase()}.${rejectionDetail}`, priority: config.priority, actionUrl: "/citizen/enrollments", actionLabel: "Ver inscripción", entityType: "enrollment", entityId: row.id, deduplicationKey: `enrollment-${target.toLowerCase()}:${row.id}:${row.updatedAt.getTime()}` }, tx);
    }
    return map(tx, row);
  });
}
