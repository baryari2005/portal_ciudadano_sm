...
const actionClassName = "h-12 rounded-xl border-[#C9D9C3] bg-[#F7FBF5] px-7 text-base font-bold text-[#1D4F36] shadow-sm hover:bg-[#EEF6E9]";
const qrOptions = { width: 420, margin: 2, color: { dark: "#003A22", light: "#FFFFFF" } };
...
export function UserQrCredentialCard({ userId, onLoadingChange }: { userId: string; onLoadingChange?: (loading: boolean) => void }) {
  ...
  return (
    <section className="rounded-[18px] border border-[#C9D9C3] bg-white/70 p-5 text-center shadow-sm">
      <h3 className="text-sm font-extrabold uppercase text-[#003A22]">QR de ingreso</h3>
      <p className="mt-1 text-xs font-medium text-[#5F6F68]">Credencial activa para recepción y asistencia</p>

      <div className="mt-4 grid min-h-56 place-items-center rounded-2xl border border-[#DDE8D7] bg-[#F7FBF5] p-3">
        {qr ? <img src={qr} alt="QR de ingreso activo" className="size-52" /> : <div className="max-w-xs py-5"><QrCode className="mx-auto size-14 text-[#819B56]" /><p className="mt-3 text-sm font-bold text-[#1D4F36]">{active ? "Reemití la credencial anterior para visualizarla" : "No hay un QR activo"}</p></div>}
      </div>
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-2 border-t border-[#DDE8D7] pt-4">
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return <div className="flex gap-2 rounded-xl bg-[#F7FBF5] p-3"><Clock3 className="mt-0.5 size-4 shrink-0 text-[#1D4F36]" /><div><dt className="text-[10px] font-bold uppercase text-[#5F6F68]">{label}</dt><dd className="text-xs font-bold text-[#173C2A]">{value}</dd></div></div>;
}
