      ...
      <div className={cn("min-h-0 flex-col gap-4", selectedId ? "hidden lg:flex" : "flex")}>
        ...
        <div className="grid gap-3 overflow-y-auto pr-1 lg:max-h-[calc(100dvh-var(--topbar-h)-290px)]">
          {filtered.map((item) => <button key={item.id} type="button" data-admin-list-card="" onClick={() => setSelectedId(item.id)} className={cn("grid w-full self-start grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#819B56]", selectedId === item.id ? "border-[#1D4F36] bg-[#EEF6E9] shadow-sm" : "border-[#DDE8D7] bg-white hover:border-[#819B56] hover:shadow-sm")}>
            {item.schedule.activity.imageUrl?<ActivityImagePreview source={item.schedule.activity.imageUrl} alt={`Imagen de ${item.schedule.activity.nombre}`} className="size-12 shrink-0 rounded-xl" />:<span className="grid size-12 place-items-center rounded-xl bg-[#1D4F36] text-white shadow-sm"><ClipboardCheck className="size-6" /></span>}
            <span className="min-w-0"><span className="flex flex-wrap items-center gap-2"><span className="truncate font-extrabold text-[#173C2A]">{item.schedule.activity.nombre}</span><EnrollmentBadge status={item.status} /></span><span className="mt-1 block text-sm text-[#315644]/75">{formatCitizenEnrollmentScheduleSummary(item)}</span><span className="mt-2 block truncate text-xs font-semibold text-[#315644]/65">{item.schedule.establishment.nombre}{item.waitlistPosition ? ` · Posición ${item.waitlistPosition}` : ""}</span></span>
...
function EnrollmentDetail({ item, onBack, onCancel, onEditSchedules }: { item: CitizenEnrollment | null; onBack: () => void; onCancel: (id: string) => void; onEditSchedules:(item:CitizenEnrollment)=>void }) {
  ...
  return <aside className="h-fit rounded-3xl border border-[#DDE8D7] bg-[#EEF6E9] p-5 text-[#173C2A] shadow-sm sm:p-7 lg:sticky lg:top-0">
    ...
    {cancellable ? <div className={`mt-6 grid gap-3 border-t border-[#C9D9C3] pt-5 ${changeable?"sm:grid-cols-2":""}`}>{changeable?<Button className="h-11 w-full rounded-xl bg-[#1D4F36] font-bold hover:bg-[#143A27]" onClick={() => onEditSchedules(item)}><Pencil />Cambiar horarios</Button>:null}<Button variant="outline" className="h-11 w-full rounded-xl border-red-200 bg-white font-bold text-red-700 hover:bg-red-50" onClick={() => onCancel(item.id)}><Trash2 />Cancelar inscripción</Button></div> : null}
  </aside>;
}
