        ...
        <section className="mt-6 grid min-h-0 gap-6 lg:grid-cols-[minmax(340px,0.95fr)_minmax(420px,1.05fr)]">
          <div className={cn("min-h-0 flex-col gap-4", selectedId ? "hidden lg:flex" : "flex")}>
            ...
            <div className="grid gap-3 overflow-y-auto pr-1 lg:max-h-[calc(100dvh-var(--topbar-h)-290px)]">
              {filtered.map((item) => (
                <button key={item.id} type="button" data-admin-list-card="" onClick={() => setSelectedId(item.id)} className={cn("grid w-full self-start grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#819B56]", selectedId === item.id ? "border-[#1D4F36] bg-[#EEF6E9] shadow-sm" : "border-[#DDE8D7] bg-white hover:border-[#819B56] hover:shadow-sm")}>
                  <AttendanceIcon status={item.status} />
                  <span className="min-w-0">
                    <span className="flex flex-wrap items-center gap-2"><span className="truncate font-extrabold text-[#173C2A]">{item.activity.nombre}</span><AttendanceBadge status={item.status} /></span>
                    <span className="mt-1 block text-sm font-semibold text-[#315644]">{formatCatalogDate(item.date)} · {item.startTime} a {item.endTime}</span>
                    <span className="mt-1 block truncate text-xs text-[#5F6F68]">{item.establishment.nombre}</span>
                  </span>
...
function AttendanceDetail({ item, onBack }: { item: CitizenAttendance | null; onBack: () => void }) {
  if (!item) return <aside className="hidden min-h-72 items-center justify-center rounded-3xl border border-[#DDE8D7] bg-[#EEF6E9] p-8 text-center text-sm font-semibold text-[#315644]/70 lg:flex">Seleccioná una asistencia para consultar su detalle.</aside>;
  return (
    <aside className="h-fit rounded-3xl border border-[#DDE8D7] bg-[#EEF6E9] p-5 text-[#173C2A] shadow-sm sm:p-7 lg:sticky lg:top-0">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1"><h2 className="break-words text-2xl font-extrabold text-[#1D4F36]">{item.activity.nombre}</h2><div className="mt-2"><AttendanceBadge status={item.status} /></div></div>
      </div>
...
function AttendanceIcon({ status, large = false }: { status: string; large?: boolean }) {
  ...
  return <span className={cn("grid shrink-0 place-items-center rounded-xl bg-[#1D4F36] text-white shadow-sm", large ? "size-16 rounded-2xl" : "size-12")}><Icon className={large ? "size-8" : "size-6"} /></span>;
}

function AttendanceBadge({ status }: { status: string }) {
  const visual = STATUS[status] ?? { label: status, className: "border-[#B2B2B2] bg-[#F2F2F2] text-[#555]" };
