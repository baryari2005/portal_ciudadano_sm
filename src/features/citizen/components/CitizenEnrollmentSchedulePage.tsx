...
import { AlertCircle, CalendarClock, CheckCircle2, FileText, Loader2, MapPin, UserRound } from "lucide-react";
...
import { CitizenMobileEnrollmentSchedule } from "./mobile/CitizenMobileEnrollmentSchedule";

type Schedule = { id:string;day:string;startTime:string;endTime:string;slotDurationMinutes:number|null;slotGapMinutes:number;establishment:{name:string}|null;professors:string[];teacherAssignments:Array<{startTime:string;endTime:string;professors:string[]}> };
type Activity = { id:string;name:string;imageUrl:string|null;schedules:Schedule[] };
...
export function CitizenEnrollmentSchedulePage({ enrollmentId, mode="citizen", returnHref }: { enrollmentId:string;mode?:"citizen"|"admin";returnHref?:string }) {
  ...
  return <main className="min-h-[calc(100dvh-var(--topbar-h)-48px)] bg-[var(--brand-page)] p-0 lg:p-8">
    {mode==="citizen"?<CitizenMobileEnrollmentSchedule activityName={activityName} imageUrl={activity.imageUrl} choices={choices} selected={selected} originalKeys={originalKeys} availability={availability} activeDay={mobileDay} checkingChoiceKey={checkingChoiceKey} choiceConflict={choiceConflict} saving={saving} hasUnavailableSelection={hasUnavailableSelection} onDayChange={setMobileDay} onToggle={toggleChoice} onCancel={()=>router.push(backHref)} onSave={()=>void save()} />:null}
    <div className={mode==="citizen"?"hidden lg:block":"block"}>
    <AdminFormHeader
      icon={CalendarClock}
...
function professorsForChoice(choice:Choice){return choice.schedule.teacherAssignments.find(item=>item.startTime===choice.startTime&&item.endTime===choice.endTime)?.professors??choice.schedule.professors;}
