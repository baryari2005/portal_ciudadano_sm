...
const controlClass = "h-11 w-full rounded-xl border-[#C9D9C3] bg-[#F7FBF5] pl-9 font-medium text-[#173C2A]";
const readOnlyClass = `${controlClass} cursor-not-allowed bg-[#EEF6E9] opacity-100`;
  ...
  return <div className="min-h-[calc(100dvh-var(--topbar-h)-48px)] w-full overflow-y-auto bg-[#F7FBF5] p-4 sm:p-6 lg:h-[calc(100dvh-var(--topbar-h)-48px)] lg:p-8"><div className="pr-2">
    ...
    <AdminWorkflowLayout sections={sections.map((item) => ({ ...item, status: stepStatus[item.id] }))} activeSection={section} onSectionChange={setSection} navigationLabel="Pasos de edición" fullWidth={!workflow}>
      <div className="rounded-3xl border border-[var(--brand-secondary)]/20 bg-white/80 p-5 text-[var(--brand-ink)] shadow-sm sm:p-6 lg:p-8">
        <header className="mb-6 border-b border-[#C9D9C3] pb-5"><h2 className="text-lg font-extrabold text-[#003A22]">{meta[0]}</h2><p className="mt-1 text-sm font-medium text-[#5F6F68]">{meta[1]}</p></header>
        <div className="space-y-6">
          {show(1) ? <div className="grid gap-x-8 gap-y-4 sm:grid-cols-2">
            <Field label="Nombre *"><IconInput id="profile-first-name" leftIcon={<User className="size-4 text-[#1D4F36]" />} input={<Input value={form.firstName} onChange={(event) => setValue("firstName", event.target.value)} onBlur={() => setValue("firstName", titleCase(form.firstName))} className={controlClass} />} /></Field>
            <Field label="Apellido *"><IconInput id="profile-last-name" leftIcon={<UserRound className="size-4 text-[#1D4F36]" />} input={<Input value={form.lastName} onChange={(event) => setValue("lastName", event.target.value)} onBlur={() => setValue("lastName", titleCase(form.lastName))} className={controlClass} />} /></Field>
            <Field label="DNI"><IconInput id="profile-document" leftIcon={<IdCard className="size-4 text-[#1D4F36]" />} input={<Input value={data.documento ?? ""} readOnly aria-readonly className={readOnlyClass} />} /></Field>
            <Field label="Fecha de nacimiento *"><Popover><PopoverTrigger asChild><Button variant="outline" type="button" className="h-11 w-full justify-start rounded-xl border-[#C9D9C3] bg-[#F7FBF5] font-medium text-[#173C2A]"><CalendarIcon className="mr-2 size-4 text-[#1D4F36]" />{form.birthDate ? format(fromYmdLocal(form.birthDate), "dd/MM/yyyy", { locale: es }) : <span className="text-muted-foreground">Seleccionar fecha</span>}</Button></PopoverTrigger><PopoverContent className="p-0" align="start"><Calendar mode="single" selected={form.birthDate ? fromYmdLocal(form.birthDate) : undefined} onSelect={(date) => setValue("birthDate", date ? toYmdLocal(date) : "")} captionLayout="dropdown" fromYear={1940} toYear={new Date().getFullYear()} /></PopoverContent></Popover></Field>
            <Field label="Edad"><IconInput id="profile-age" leftIcon={<CakeSlice className="size-4 text-[#1D4F36]" />} input={<Input value={age === null ? "Se calcula automáticamente" : `${age} años`} readOnly className={readOnlyClass} />} /></Field>
            <Field label="Nacionalidad *"><IconInput id="profile-nationality" leftIcon={<Map className="size-4 text-[#1D4F36]" />} input={<Select value={form.nationality} onValueChange={(value: Nacionalidad) => setValue("nationality", value)}><SelectTrigger className={controlClass}><SelectValue placeholder="Seleccionar" /></SelectTrigger><SelectContent>{NACIONALIDAD_VALUES.map((item) => <SelectItem key={item} value={item}>{item.replaceAll("_", " ")}</SelectItem>)}</SelectContent></Select>} /></Field>
            <Field label="Sexo / género *"><IconInput id="profile-gender" leftIcon={<UserRound className="size-4 text-[#1D4F36]" />} input={<Select value={form.gender} onValueChange={(value: Genero) => setValue("gender", value)}><SelectTrigger className={controlClass}><SelectValue placeholder="Seleccionar" /></SelectTrigger><SelectContent>{GENERO_OPCIONES.map((item) => <SelectItem key={item} value={item}>{item.replaceAll("_", " ")}</SelectItem>)}</SelectContent></Select>} /></Field>
          </div> : null}
          {show(2) ? <div className="space-y-4"><div className="grid gap-x-8 gap-y-4 sm:grid-cols-2"><Field label="User ID"><IconInput id="profile-user-id" leftIcon={<User className="size-4 text-[#1D4F36]" />} input={<Input value={data.userId} readOnly aria-readonly className={readOnlyClass} />} /></Field><div className="flex items-end"><Button type="button" className={`${adminPrimaryButtonClass} w-full gap-2`} onClick={() => setPasswordDialogOpen(true)}><KeyRound />Cambiar contraseña</Button></div></div></div> : null}
          {show(3) ? <div className="grid gap-x-8 gap-y-4 sm:grid-cols-2"><Field label="Dirección *" className="sm:col-span-2"><GoogleAddressInput id="profile-address" value={form.address} placeId={form.addressPlaceId} onChange={(location) => { setForm((current) => ({ ...current, address: location.address, addressPlaceId: location.placeId, addressLat: location.lat, addressLng: location.lng })); markUnsaved(3); }} className={controlClass} placeholder="Ej: Av. Presidente Perón 1234" /></Field><Field label="Localidad *"><IconInput id="profile-locality" leftIcon={<MapPinned className="size-4 text-[#1D4F36]" />} input={<Input value={form.locality} onChange={(event) => setValue("locality", event.target.value)} className={controlClass} />} /></Field><Field label="Provincia *"><IconInput id="profile-province" leftIcon={<Map className="size-4 text-[#1D4F36]" />} input={<Select value={form.province} onValueChange={(value) => setValue("province", value)}><SelectTrigger className={controlClass}><SelectValue placeholder="Seleccionar provincia" /></SelectTrigger><SelectContent>{ARGENTINA_PROVINCES.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select>} /></Field><Field label="Código postal *"><IconInput id="profile-postal-code" leftIcon={<IdCard className="size-4 text-[#1D4F36]" />} input={<Input value={form.postalCode} onChange={(event) => setValue("postalCode", event.target.value)} className={controlClass} />} /></Field></div> : null}
          {show(4) ? <div className="grid gap-x-8 gap-y-4 sm:grid-cols-2"><Field label="Email"><IconInput id="profile-email" leftIcon={<Mail className="size-4 text-[#1D4F36]" />} input={<Input value={data.email} readOnly aria-readonly className={readOnlyClass} />} /></Field><Field label="Teléfono *"><PhoneInput id="profile-phone" value={form.phone} onChange={(value) => setValue("phone", value)} className={controlClass} required /></Field><Field label="Persona de contacto de emergencia *"><IconInput id="profile-emergency-name" leftIcon={<Contact className="size-4 text-[#1D4F36]" />} input={<Input value={form.emergencyContactName} onChange={(event) => setValue("emergencyContactName", event.target.value)} className={controlClass} />} /></Field><Field label="Teléfono de emergencia *"><PhoneInput id="profile-emergency-phone" value={form.emergencyContactPhone} onChange={(value) => setValue("emergencyContactPhone", value)} className={controlClass} required /></Field></div> : null}
          {show(5) ? <div className="grid gap-x-8 gap-y-4 sm:grid-cols-2"><Field label="Obra social o prepaga" className="sm:col-span-2"><IconInput id="profile-coverage" leftIcon={<HeartPulse className="size-4 text-[#1D4F36]" />} input={<MedicalCoverageSelect value={form.medicalCoverageId} onChange={(medicalCoverageId) => { setForm((current) => ({ ...current, medicalCoverageId })); markUnsaved(5); }} triggerClassName="pl-9" />} /></Field><Field label="Número de afiliado" className="sm:col-span-2"><IconInput id="profile-affiliate" leftIcon={<IdCard className="size-4 text-[#1D4F36]" />} input={<Input value={form.affiliateNumber} onChange={(event) => setValue("affiliateNumber", event.target.value)} className={controlClass} />} /></Field></div> : null}
...
function Field({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return <div className={`space-y-1 ${className ?? ""}`}><Label className="font-extrabold text-[#173C2A]">{label}</Label>{children}</div>;
}
