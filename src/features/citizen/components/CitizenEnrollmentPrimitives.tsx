...
export function EnrollmentBadge({status}:{status:string}){return <Badge variant="outline" className={cn("w-fit rounded-full px-2.5 py-1 font-bold",status==="CONFIRMADA"?"border-[var(--brand-secondary)]/40 bg-[var(--brand-highlight)] text-[var(--brand-primary)]":status==="PENDIENTE"||status==="LISTA_ESPERA"?"border-amber-300 bg-amber-50 text-amber-900":"border-[var(--brand-neutral)] bg-[var(--brand-neutral)]/15 text-[#555]")}>{labels[status]??status}</Badge>}
