...
const ALL_ESTABLISHMENTS = "__all__";

export function WorkspaceEstablishmentSelector({ compact = false, className }: { compact?: boolean; className?: string }) {
  const { workspace, options, establishmentId, setEstablishmentId, selected } = useWorkspaceEstablishment();
  const showAllOption = workspace === "teacher";
  return (
    <div className={cn(compact ? "flex min-w-0 items-center gap-2" : "hidden items-center gap-2 xl:flex", className)}>
      ...
      <Select
        value={showAllOption && !establishmentId ? ALL_ESTABLISHMENTS : establishmentId}
        onValueChange={(value) => setEstablishmentId(value === ALL_ESTABLISHMENTS ? "" : value)}
      >
        <SelectTrigger
          size={compact ? "sm" : "default"}
          aria-label="Establecimiento"
          title={showAllOption && !establishmentId ? "Todas las sedes" : selected?.nombre}
        ...
        <SelectContent>
          {showAllOption ? <SelectItem value={ALL_ESTABLISHMENTS}>Todas las sedes</SelectItem> : null}
          {options.map((item) => <SelectItem key={item.id} value={item.id}>{item.nombre}</SelectItem>)}
        </SelectContent>
      </Select>
    </div>
  );
