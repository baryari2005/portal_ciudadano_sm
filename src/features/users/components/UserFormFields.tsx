    ...
    <div className="rounded-3xl border border-[var(--brand-secondary)]/20 bg-white/80 p-5 text-[var(--brand-ink)] shadow-sm sm:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        {step == null || (step != null && [1, 3, 4, 5].includes(step)) ? <div className="grid gap-x-8 gap-y-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2"><Controller control={control} name="domicilio" render={({field})=><GoogleAddressInput display="map" id="domicilio-map" value={field.value??""} placeId={watch("domicilioPlaceId")} lat={watch("domicilioLat")} lng={watch("domicilioLng")} locality={watch("localidad")} province={watch("provincia")} postalCode={watch("codigoPostal")} onChange={(location)=>{field.onChange(location.address);setValue("domicilioPlaceId",location.placeId);setValue("domicilioLat",location.lat);setValue("domicilioLng",location.lng);if(location.locality)setValue("localidad",location.locality);if(location.province)setValue("provincia",location.province);if(location.postalCode)setValue("codigoPostal",location.postalCode)}} />}/></div>
          </> : null}
