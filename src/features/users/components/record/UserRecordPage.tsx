...
function ParticipationSection({ user, onSaved }: { user: ManagedUser; onSaved: () => Promise<void> }) {
  ...
  return <RecordCard title="Participación" description="Control administrativo de ausencias y habilitación del ciudadano."><div className="grid gap-4 lg:grid-cols-3"><div className="space-y-2"><Label>Ausencias justificadas</Label><div className="relative"><CircleCheck className="pointer-events-none absolute left-3 top-1/2 z-10 size-4 -translate-y-1/2 text-[#1D4F36]" /><Input type="number" min={1} value={justified} onChange={(event) => setJustified(event.target.value)} className="h-11 rounded-xl border-[#C9D9C3] bg-white pl-10 shadow-sm" /></div></div><div className="space-y-2"><Label>Ausencias injustificadas</Label><div className="relative"><CircleX className="pointer-events-none absolute left-3 top-1/2 z-10 size-4 -translate-y-1/2 text-[#1D4F36]" /><Input type="number" min={1} value={unjustified} onChange={(event) => setUnjustified(event.target.value)} className="h-11 rounded-xl border-[#C9D9C3] bg-white pl-10 shadow-sm" /></div></div><div className="space-y-2"><Label>Estado de participación</Label><div className="flex h-11 items-center justify-between rounded-xl border border-[#C9D9C3] bg-white px-4 shadow-sm"><span className="text-sm font-bold text-[#1D4F36]">{participationEnabled ? "Activa" : status === "EN_REVISION" ? "En revisión" : "Inactiva"}</span><Switch checked={participationEnabled} onCheckedChange={(checked) => setStatus(checked ? "HABILITADO" : "SUSPENDIDO_PROVISORIO")} aria-label="Activar o desactivar participación" /></div></div><div className="space-y-2 lg:col-span-3"><Label>Observaciones administrativas</Label><div className="relative"><MessageSquareText className="pointer-events-none absolute left-3 top-3 z-10 size-4 text-[#1D4F36]" /><Textarea value={observations} onChange={(event) => setObservations(event.target.value)} rows={5} className="resize-none rounded-xl border-[#C9D9C3] bg-white pl-10 shadow-sm" placeholder="Agregá información relevante sobre la participación del ciudadano..." /></div></div></div><Separator className="my-6 bg-[#C9D9C3]" /><Button disabled={saving} className="h-12 rounded-xl bg-[#1D4F36] px-7 text-base font-bold text-white hover:bg-[#143A27]" onClick={() => void save()}>{saving ? <Loader2 className="animate-spin" /> : <Save />}Guardar política</Button></RecordCard>;
}
