...
export async function updateGeneralSettings(input: GeneralSettings, actorId: string) {
  ...
  const row = await prisma.parametrosGenerales.upsert({
    where: { id: 1 },
    update: { registrosPorPagina: input.pageSize, imagenLoginCollage1: input.loginCollageImages[0], imagenLoginCollage2: input.loginCollageImages[1], imagenLoginCollage3: input.loginCollageImages[2], imagenLoginCollage4: input.loginCollageImages[3], updatedById: actorId },
    create: { id: 1, registrosPorPagina: input.pageSize, imagenLoginCollage1: input.loginCollageImages[0], imagenLoginCollage2: input.loginCollageImages[1], imagenLoginCollage3: input.loginCollageImages[2], imagenLoginCollage4: input.loginCollageImages[3], updatedById: actorId },
  });
  const next = map(row);
