"use client";
import { BadgeDollarSign, FileText, GraduationCap, NotebookText, Tags } from "lucide-react";
...
const controlClass = "h-12 w-full rounded-xl border-[var(--brand-border)] bg-[var(--brand-page)] pl-11 font-medium text-[var(--brand-ink)] placeholder:text-[#6D8D75]";
export function GeneralInformation({ payload, patch, categories }: { payload: ActivityDraftPayload; patch: (value: Partial<ActivityDraftPayload>) => void; categories: Array<{ id: string; nombre: string }> }) {
  return <div className="space-y-6"><RequestAccessPhotoField currentUrl={payload.imagenUrl} title="Foto de la actividad" description="Referencia visual que se mostrará en el portal ciudadano." uploadEndpoint="/api/activities/images" allowCamera onUploaded={({ publicUrl }) => patch({ imagenUrl: publicUrl })} onClear={() => patch({ imagenUrl: null })} sidePreview /><div className="grid gap-5 sm:grid-cols-2">
    ...
    <Control label="Descripción" icon={<NotebookText />} wide><Textarea className="min-h-28 w-full rounded-xl border-[var(--brand-border)] bg-[var(--brand-page)] pl-11 pt-3 text-[var(--brand-ink)]" value={payload.descripcion ?? ""} onChange={(event) => patch({ descripcion: event.target.value })} /></Control>
    ...
    <label className="flex h-12 cursor-pointer items-center gap-3 self-end rounded-xl border border-[var(--brand-border-soft)] bg-[var(--brand-page)] px-4"><Checkbox checked={payload.esGratuita} onCheckedChange={(value) => patch({ esGratuita: value === true, precio: value === true ? null : payload.precio })} /><span className="font-bold text-[var(--brand-ink)]">Actividad gratuita</span></label>
  </div></div>;
}
function Control({ label, icon, wide, children }: { label: string; icon: React.ReactNode; wide?: boolean; children: React.ReactNode }) { return <div className={`space-y-2 ${wide ? "sm:col-span-2" : ""}`}><Label className="font-bold text-[var(--brand-ink)]">{label}</Label><div className="relative"><span className="pointer-events-none absolute left-3.5 top-3.5 z-10 text-[var(--brand-primary)] [&_svg]:size-5">{icon}</span>{children}</div></div>; }
