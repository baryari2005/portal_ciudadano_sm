...
export async function saveDraftClient(id: string, payload: ActivityDraftPayload, currentStep: number, expectedUpdatedAt: string) { const { data } = await axiosInstance.patch<{ data: ActivityDraft }>(`/activity-drafts/${id}`, { payload, pasoActual: currentStep, expectedUpdatedAt }); return data.data; }
export async function publishDraftClient(id: string, expectedUpdatedAt: string) { try { const { data } = await axiosInstance.post<{ data: { id: string } }>(`/activity-drafts/${id}/publish`, { expectedUpdatedAt }); return data.data; } catch (error) { throw new Error(getAxiosMessage(error, "No pudimos publicar la actividad.")); } }
...
export async function discardDraftClient(id: string, expectedUpdatedAt: string){await axiosInstance.delete(`/activity-drafts/${id}`, { data: { expectedUpdatedAt } });}
