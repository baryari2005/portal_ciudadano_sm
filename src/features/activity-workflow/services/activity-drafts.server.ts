...
export async function publishActivityDraft(id: string, userId: string) {
  ...
  if (draft.activityId) {
    ...
    if(scheduleStructureChanged)changes.horarios=p.schedules.map(schedule=>({id:schedule.id,diaSemana:schedule.diaSemana,horaInicio:schedule.horaInicio,horaFin:schedule.horaFin}));
