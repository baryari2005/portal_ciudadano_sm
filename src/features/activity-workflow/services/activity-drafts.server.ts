...
export async function createActivityEditDraft(activityId: string, userId: string) {
  ...
  const payload = activityDraftPayloadSchema.parse({
    ...
    fechasExcluidas: [],
    schedules: activity.horarios.map((schedule) => ({ id: schedule.id, establecimientoId: schedule.establecimientoId, diaSemana: schedule.diaSemana, horaInicio: schedule.horaInicio, horaFin: schedule.horaFin, espacio: schedule.espacio, cupoMaximo: schedule.cupoMaximo, duracionTurnoMinutos: schedule.duracionTurnoMinutos ?? activity.duracionTurnoMinutos, intervaloTurnoMinutos: schedule.duracionTurnoMinutos ? schedule.intervaloTurnoMinutos : activity.intervaloTurnoMinutos, profesorIds: schedule.profesores.map((item) => item.profesorId), recursoIds: schedule.recursos.map((item) => item.recursoId), teacherAssignments: [...new Map(schedule.clases.map((item) => [`${item.horaInicio}:${item.horaFin}`, { startTime: item.horaInicio, endTime: item.horaFin, professorIds: item.profesores.map((link) => link.profesorId) }])).values()] })),
  });
...
async function publishActivityDraftContents(id: string, userId: string) {
  ...
  if (draft.activityId) {
    ...
    const normalizeScheduleStructure=(items:any[])=>items.map(item=>({id:item.id??null,establecimientoId:item.establecimientoId,diaSemana:item.diaSemana,horaInicio:item.horaInicio,horaFin:item.horaFin})).sort((a,b)=>`${a.id}${a.diaSemana}${a.establecimientoId}`.localeCompare(`${b.id}${b.diaSemana}${b.establecimientoId}`));
    const normalizeSchedules=(items:any[])=>items.map(item=>({id:item.id??null,establecimientoId:item.establecimientoId,diaSemana:item.diaSemana,horaInicio:item.horaInicio,horaFin:item.horaFin,espacio:item.espacio??null,cupoMaximo:item.cupoMaximo,duracionTurnoMinutos:item.duracionTurnoMinutos??null,intervaloTurnoMinutos:item.intervaloTurnoMinutos??0,profesorIds:[...new Set<string>(item.profesorIds??item.profesores?.map((link:any)=>link.profesorId)??[])].sort(),recursoIds:[...new Set<string>(item.recursoIds??item.recursos?.map((link:any)=>link.recursoId)??[])].sort(),teacherAssignments:(item.teacherAssignments??[...new Map((item.clases??[]).map((session:any)=>[`${session.horaInicio}:${session.horaFin}`,{startTime:session.horaInicio,endTime:session.horaFin,professorIds:session.profesores.map((link:any)=>link.profesorId)}])).values()]).map((assignment:any)=>({...assignment,professorIds:[...assignment.professorIds].sort()})).sort((a:any,b:any)=>a.startTime.localeCompare(b.startTime))})).sort((a,b)=>`${a.id}${a.diaSemana}${a.establecimientoId}`.localeCompare(`${b.id}${b.diaSemana}${b.establecimientoId}`));
    ...
    if(schedulesChanged)for(const schedule of p.schedules){const usesTurns=["TURNO_RECURRENTE","TURNO_PUNTUAL"].includes(p.modalidadOperacion);for(const assignment of effectiveTeacherAssignments(schedule,schedule.duracionTurnoMinutos,schedule.intervaloTurnoMinutos,usesTurns))await assertActivityScheduleAvailability({establishmentId:schedule.establecimientoId,diaSemana:schedule.diaSemana,horaInicio:assignment.startTime,horaFin:assignment.endTime,espacio:null,profesoresIds:assignment.professorIds,recursos:[],excludeId:schedule.id});await assertActivityScheduleAvailability({establishmentId:schedule.establecimientoId,diaSemana:schedule.diaSemana,horaInicio:schedule.horaInicio,horaFin:schedule.horaFin,espacio:schedule.espacio,profesoresIds:[],recursos:schedule.recursoIds.map(recursoId=>({recursoId,cantidadReservada:1,exclusivo:false})),excludeId:schedule.id});}
    const changes:any={};
    ...
    if(!activity)throw new CatalogNotFoundError("Actividad no encontrada.");
    if(schedulesChanged){const used=new Set<string>();for(const schedule of p.schedules){const target=schedule.id?activity.horarios.find(item=>item.id===schedule.id):activity.horarios.find(item=>!used.has(item.id!)&&item.diaSemana===schedule.diaSemana&&item.horaInicio===schedule.horaInicio&&item.horaFin===schedule.horaFin&&item.establecimientoId===schedule.establecimientoId);if(!target?.id)throw new CatalogValidationError("No pudimos vincular uno de los horarios actualizados.");used.add(target.id);const previous=current.horarios.find(item=>item.id===target.id);const updated=await updateActivitySchedule(target.id,{establecimientoId:schedule.establecimientoId,espacio:schedule.espacio,cupoMaximo:schedule.cupoMaximo,profesoresIds:schedule.profesorIds,profesorPrincipalId:schedule.profesorIds[0]??null,duracionTurnoMinutos:schedule.duracionTurnoMinutos,intervaloTurnoMinutos:schedule.intervaloTurnoMinutos,recursos:schedule.recursoIds.map(recursoId=>({recursoId,cantidadReservada:1,estrategiaAsignacion:"AL_INGRESAR",exclusivo:false}))},undefined,{ignoreProfessorConflicts:true});const usesTurns=["TURNO_RECURRENTE","TURNO_PUNTUAL"].includes(p.modalidadOperacion);await syncGeneratedSessionTeachers(target.id,effectiveTeacherAssignments(schedule,schedule.duracionTurnoMinutos,schedule.intervaloTurnoMinutos,usesTurns));const professorUsers=await prisma.profesor.findMany({where:{id:{in:updated.professors.map((item:{id:string})=>item.id)}},select:{id:true,usuarioId:true}}),userByProfessor=new Map(professorUsers.map(item=>[item.id,item.usuarioId]));await notifyTeacherAssignmentChanges(prisma,{previous:(previous?.profesores??[]).map(item=>({professorId:item.profesorId,isPrimary:item.esPrincipal,userId:userByProfessor.get(item.profesorId)??""})),current:updated.professors.map((item:{id:string;isPrimary:boolean})=>({professorId:item.id,isPrimary:item.isPrimary,userId:userByProfessor.get(item.id)??""})),context:{kind:"schedule",entityId:updated.id,activityName:updated.activity.name,establishmentId:updated.establishmentId,establishmentName:updated.establishment.name,space:updated.space,day:updated.day,startTime:updated.startTime,endTime:updated.endTime},senderId:userId,operationKey:`workflow:${id}:${new Date(draft.updatedAt).getTime()}`});}}
    await prisma.actividadBorrador.update({ where: { id }, data: { publicadoAt: new Date(), estado: "PUBLICADO", creadoPorId: userId } });
    return activity;
  }
