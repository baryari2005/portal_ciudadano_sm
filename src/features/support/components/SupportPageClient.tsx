...
function GuideCard({ guide }: { guide: HelpGuide }) {
  ...
  return (
    <Card id={guide.id} className="scroll-mt-24 border-[var(--brand-border-soft)] bg-white shadow-sm">
      <CardHeader className="pb-4">
        <CardTitle className="text-xl font-extrabold text-[var(--brand-primary)]">{guide.title}</CardTitle>
      ...
      <CardContent className="space-y-5">
        ...
        {guide.warnings?.length ? <div className="rounded-2xl border border-[var(--brand-secondary)]/30 bg-[#F3F8EF] p-4"><div className="flex items-center gap-2 font-bold text-[var(--brand-primary)]"><CircleAlert className="size-4"/>Tené en cuenta</div><ul className="mt-2 space-y-1 text-sm leading-6 text-[#496557]">{guide.warnings.map((warning) => <li key={warning}>• {warning}</li>)}</ul></div> : null}
        {links.length ? <div className="flex flex-wrap gap-2">{links.map((link) => <Button key={link.href} asChild variant="outline" className="border-[var(--brand-secondary)]/40 text-[var(--brand-primary)] hover:bg-[var(--brand-panel)]"><Link href={link.href}>{link.label}<ChevronRight className="size-4"/></Link></Button>)}</div> : null}
      </CardContent>
    </Card>
  );
}
...
export function SupportPageClient() {
  ...
  return <main className="min-h-full bg-[var(--brand-page)] p-4 sm:p-6 lg:p-8">
    <section className="overflow-hidden rounded-3xl bg-[var(--brand-primary)] px-6 py-8 text-white shadow-lg sm:px-8">
      <Badge className="bg-[var(--brand-secondary)] text-white">Centro de ayuda</Badge>
      ...
      <div className="relative mt-6 max-w-2xl"><Search className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-[var(--brand-muted)]"/><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por tarea, pantalla o tema..." className="h-12 border-white/20 bg-white pl-12 text-[var(--brand-ink)] placeholder:text-[#718078]"/></div>
    </section>

    {guides.length ? <div className="mt-6 grid items-start gap-5 xl:grid-cols-2">{guides.map((guide) => <GuideCard key={guide.id} guide={guide}/>)}</div> : <Card className="mt-6 border-[var(--brand-border-soft)]"><CardContent className="py-10 text-center text-[var(--brand-muted)]">No encontramos guías de esta experiencia que coincidan con tu búsqueda y tus permisos.</CardContent></Card>}
