...
export function ActividadesPage() {
  ...
  async function discardDraft(){if(!draftToDiscard)return;setDiscardingDraft(true);try{await discardDraftClient(draftToDiscard.id, draftToDiscard.updatedAt);setDrafts(current=>current.filter(item=>item.id!==draftToDiscard.id));if(selectedDraft?.id===draftToDiscard.id)setSelectedDraft(null);toast.success("Borrador descartado.");}catch(error){toast.error(getAxiosMessage(error, "No pudimos descartar el borrador."));}finally{setDiscardingDraft(false);setDraftToDiscard(null);}}
