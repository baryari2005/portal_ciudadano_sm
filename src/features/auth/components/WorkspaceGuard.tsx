...
import { getDefaultWorkspace, hasAccessWorkspace, hasAdministrativeWorkspace, hasTeacherPermissions } from "../libs/workspaces";
export function WorkspaceGuard({ workspace, children }: { workspace: "administration" | "access" | "teacher"; children: React.ReactNode }) {
  ...
  const allowed = workspace === "administration" ? hasAdministrativeWorkspace(user) : workspace === "access" ? hasAccessWorkspace(user) : hasTeacherPermissions(user) && teacherEnabled;
